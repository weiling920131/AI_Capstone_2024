...
class AlphaZero(nn.Module):
    def __init__(self, tensor_shape, channels, blocks, num_players, num_distinct_actions):
                #  game,
                #  config):
        super().__init__()
        self.observation_tensor_shape = tensor_shape
        ...
        self.policy_head_end = nn.Sequential(
            nn.Linear(in_features=2 * height * width,
                      out_features=num_distinct_actions),
            nn.Softmax(dim=1)
        )
