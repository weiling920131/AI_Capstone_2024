...
class ReplayBuffer(Dataset):
    def __init__(self, game, config):
        ...
        self.capacity = 16 *16 *16 *16 *10 # config['replay_buffer_size']
        self.states_to_train = 16 *16 *16 *16 # config.get('states_to_train', None)
        self.trajectories_to_train = None # config.get('trajectories_to_train', None)
        assert self.states_to_train != self.trajectories_to_train, 'the two options are disjoint.'
        self.sample_ratio = 1 # config.get('sample_ratio', None)
        self.sample_states = None # config.get('sample_states', None)
        assert self.sample_ratio != self.sample_states, 'the two options are disjoint.'
        self.sample_retention = True # config.get('sample_retention', False)

        self.use_observation = False # config.get('use_observation', False)
        self.use_transformation = False # config.get('use_transformation', False)
    ...
    def add_trajectory(self, trajectory):
        if not self.use_observation:
            state = Sample.Node() # self.game.new_initial_state()
        for pb_state in trajectory.states:
            if not self.use_observation:
                observation_tensor = torch.tensor(state.observation_tensor()).view(state.tensor_shape)
                Sample.applyStep(state.playerID, state.mapStat, state.sheepStat, pb_state.transition.action) # state.apply_action(pb_state.transition.action)
            else:
                observation_tensor = torch.tensor(pb_state.observation_tensor).view(state.tensor_shape)
...
class Learner:
    def __init__(self, args, config, tensor_shape, channels, blocks, num_players, num_distinct_actions):
        self.tensro_shape = tensor_shape
        ...
        self.game = 'battleSheep' # clap.game.load(config['game'])
        ...
        self.model_dir = self.model_dir / f"battlesheep{Sample.boardSize}_{Sample.sheepNum}"
        ...
        self.checkpoint_freq = 10 # config['train']['checkpoint_freq']
        ...
        self.batch_size = 1000 # config['train']['batch_size']
        ...
        self.model = AlphaZero(tensor_shape, channels, blocks, num_players, num_distinct_actions)
    ...
    def save_model(self):
        model = AlphaZero(self.game, self.tensor_shape, self.channels, self.blocks, self.num_players, self.num_distinct_actions)
        model.load_state_dict(self.model.module.state_dict())
        model.eval()
        model_path = self.model_dir / f'{self.iteration:05d}.pt'
        model_path = self.model_dir / f'{self.iteration:05d}.pt'

        torch.save({'model': model, 'game': Sample.tensor_shape}, model_path)
...
async def main():
    ...
    learner = Learner(args, config=config, tensor_shape=Sample.tensor_shape, channels=64, blocks=10, num_players=4, num_distinct_actions=Sample.num_distinct_actions)
    await learner.loop()
